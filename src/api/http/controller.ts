...
export class HTTPController {
  constructor(
    @inject(TYPES.AddItemToCartUseCase)
    private _addItemToCartUseCase: AddItemToCartUseCase,
    @inject(TYPES.GetCartUseCase) private _getCartUseCase: GetCartUseCase,
    @inject(TYPES.EditCartItemUseCase)
    private _editCartItemUseCase: EditCartItemUseCase,
    @inject(TYPES.RemoveItemFromCartUseCase)
    ...
    @inject(TYPES.UpdateItemStockUseCase)
    private _updateItemInStockUseCase: UpdateItemInStockUseCase,
    @inject(TYPES.AddItemToStockUseCase)
    private _addItemToStockUseCase: AddItemToStockUseCase,
    @inject(TYPES.GetItemByIdUseCase)
    private _getItemByIdUseCase: GetItemByIdUseCase
  ) {}
  ...
  public async editCartItem(ctx: RouterContext): Promise<void> {
    ...
    const itemInCart = cartCheck.items.find((item) => item.id === itemId);
    const modifyQty = itemInCart.quantity + quantity;
    if (modifyQty < 0) {
      throw new Error(`Modify quantity exceed number in cart`);
    }

    const cart = await this._editCartItemUseCase.execute(
      cartId,
      itemId,
      modifyQty
    );
